...
State::State(const char* fileName)
{
	...
	// 创建显示图标
	image = new Image("./resources/nimotsuKunImage2.dds");
	// 获取场景有效数据
	unsigned mSizeX = 0;
	unsigned mSizeY = 0;
...
{
	// 现在开始显示图像  双图层 终点图层  实体图层  3.3 三图层 mGoal mArray2D 颜色覆盖
	...
	for (unsigned ty = 0; ty < iStateSizeY; ty++)
	{
		for (unsigned tx = 0; tx < iStateSizeX; tx++)
		{
			...
			Object obj = mArray2D(tx, ty);
			if (obj != OBJ_WALL)
			{
				tImageMapperIdX = (mGoalFlag(tx, ty)) ? IMAGE_ID_GOAL : IMAGE_ID_SPACE;
			...
			tImageMapperIdX = IMAGE_ID_SPACE;
			//// 绘制玩家 和物体
			...
			if (tImageMapperIdX != IMAGE_ID_SPACE)
			{
				image->imageCellDraw(
					tImageMapperIdX * 32, 0,
					(tImageMapperIdX + 1) * 32, 32,
