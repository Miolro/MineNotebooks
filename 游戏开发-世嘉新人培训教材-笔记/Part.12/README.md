...
public:
	...
	void operator-=(const Vector&);	// 向量减法
	void operator=(const Vector&);	// 向量复制
	void setInterpolation(Vector& a, const Vector& ab, double absize, const Vector& ac, double acsize);// 向量补间
	//向量 
	double x;
	double y;
};
...
Vector::Vector(double ix, double iy) :x(ix), y(iy) {}
Vector::~Vector() {}
void Vector::operator*=(double in)
{
	x *= in;	y *= in;
}
void Vector::operator+=(const Vector& thatVector)
...
void Vector::setInterpolation(Vector& a, const Vector& ab, double absize, const Vector& ac, double acsize)
{
...
namespace GameLib
{
	...
	void transformPosition(Vector* oVector, const Vector& iVector, const Vector& offset, const Matrix22 matrix)
	{
		//初始化outVector 使用operator=来深拷贝数值
		*oVector = iVector;
		// 偏移到圆心
	...
	void Framework::update() {
		...
		Vector Oo(50.0, 50.0);								// 原点坐标
		// 三个顶点的坐标
		transformPosition(&Oa, Oa, Oo, matrix22);
		transformPosition(&Ob, Ob, Oo, matrix22);
		transformPosition(&Oc, Oc, Oo, matrix22);
		...
		for (int y = 0; y < 100; y++)
		{
			for (int x = 0; x < 100; x++)
			{
				...
				double proportion1 = static_cast<double>(x) / 100.0;// ab向量的比例
				double proportion2 = static_cast<double>(y) / 100.0;// ac向量的比例
				// 补间
				Vector p;
				//p.setInterpolation(Oa, Ob, proportion1, Oc, proportion2);  // 阴暗的旋转
				p.setInterpolation(Oa, ab, proportion1, ac, proportion2);  // 正常的旋转

				int tx = round(p.x);
				int ty = round(p.y);
				if (ty > 0 && tx > 0)
				{
					vram[ty * width() + tx] = 0xffffffff;
				}
			}
		}
		vram[50 * width() + 50] = 0xffff0000;
		++gCount;
	}
}
```
...
`需要注意一下你在旋转点了之后他们的向量也是跟着一起旋转的所以需要求出新的向量ab和ac`

`如果三个点旋转了但是不转动向量会造成诡异扭曲的旋转`
...
```C++
	p.setInterpolation(Oa, Ob, proportion1, Oc, proportion2);  // 
```

`理解书中话:总长为100遍历0-100相当于走路然后可以通过static_cast<double>(x) / 100.0算出现在所在的像素位置 然后通过旋转后的Oa点 Ob点 Oc点分别乘上他们的比例来得到p点`

`长话短说就是:你往右走了几步(或者说几分之几)又往下走了几步所在的位置就是你要输出的像素点`
...
虽然还是需要算出每个像素点的位置但是不用对每个像素点使用矩阵旋转

这边笔记口述的比较少建议参照书来看,markdown不方便描述
...
namespace GameLib
{
	...
	void Framework::update() {
		if (init) {
			init = false;
			vram = videoMemory();
			Image image("E:/environment/GameLib/src/03_2DGraphics2/displayImage/bar.dds");
		...
		Matrix22 matrix22(cos(gCount), -sin(gCount), sin(gCount), cos(gCount));
		// 对角点进行旋转
		Vector Oa(0, 0), Ob(gImageWidth + 0.00, 0.0), Oc(0.0, gImageHeight + 0.00); 	// 首先是需要三个角点  分别是(0,0) (4,0) (0,4)
		Vector Oo(gImageWidth / 2 * scaling, gImageHeight / 2 * scaling);								// 原点坐标
		 // 移动
		Vector temp(0, 0);
		temp = Oa;
		temp *= scaling;
		...
		for (int y = 0; y < gImageHeight; y++)
		{
			...
			{
				// 缩放
				double proportion1 = static_cast<double>(x) / gImageWidth * scaling;// ab向量的比例  // 修改比例做到缩放
				double proportion2 = static_cast<double>(y) / gImageHeight * scaling;// ac向量的比例  // 修改比例做到缩放
				// 补间
				Vector p;
				//p.setInterpolation(Oa, Ob, proportion1, Oc, proportion2);  // 阴暗的旋转
				// 补间渲染
